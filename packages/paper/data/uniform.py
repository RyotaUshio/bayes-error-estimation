...
def generate_uniform_data(n_samples, n_hard_labels, label_flip_rate, weights):
    ...
    soft_labels_clean = np.where(
        original_labels == 1, 1 - label_flip_rate, label_flip_rate
    )
    soft_labels_hard = generate_approximate_soft_labels(
        rng, soft_labels_clean, n_hard_labels
    )
    return {
...
def test():
    ...
    n_coincide = np.count_nonzero(
        data['labels'] == np.where(data['soft_labels_hard'] > 0.5, 1, 0)
    )
    ...
    assert np.all(data['soft_labels_hard'] >= 0) and np.all(
        data['soft_labels_hard'] <= 1
    )
